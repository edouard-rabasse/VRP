...
#SBATCH --output=logs/refineEasy-%A_%a.log
...
MAIN_CLASS="main.Main_refineEasy"
BIN_DIR="bin"
SRC_DIR="src"
...
CONFIG_FILE="configuration7_easy.xml"
# OTHER_ARGS="1 8"
...
javac  -cp "$CP_COMPILE"  -d "$BIN_DIR"  src/main/Main_refineEasy.java
...
java \
  -Djava.library.path="$GUROBI_TMP_LIB" \
  -cp "$BIN_DIR:$JAR" \
  "$MAIN_CLASS" "Coordinates_${i}.txt" "Arcs_${i}_1.txt" "$CONFIG_FILE"
done