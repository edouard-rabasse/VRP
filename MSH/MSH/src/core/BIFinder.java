...
public class BIFinder extends InsertionFinder {
	...
	/**
	 * If the distance to the node being considered is shorter than the current
	 * search bound
	 * the bound should be updated (we are looking for the nearest neighbor to an
	 * already routed node).
	 */
	@Override
	...
	/**
	 * Sort the list of candidate nodes in non-decreasing order and select the kth
	 * candidate
	 */
	@Override
	 ...
	 *         It is assumed that route r has the following structure
	 *         r={0,i-1,i,i+1,0}. In other words it is assumed
	 *         that route r starts and ends at the depot.
	 */
	public Insertion findInsertion(Route r, Hashtable<Integer, Integer> routedNodes,
			Hashtable<Integer, Integer> nodesToRoute, int k) {
		...
		while (it.hasNext()) {
			...
			for (int i = 0; i < distances.size(); i++) {
				...
				if (nodeToInsert != tailNode && nodeToInsert != headNode) {
					double cost = this.distances.getDistance(tailNode, nodeToInsert);// From the tail node to the
																						// inserting node
					cost = cost + this.distances.getDistance(nodeToInsert, headNode); // From the inserting to the head
																						// node
			...
			if (!currentArc.isInsertionEmpty()) {
				currentArc.sortInsertions();

				// Sort the insertions for the current arc
			} else {
				System.out.println("[debug] No insertions found for arc: " + currentArc.getKey());
			}
		}
	}
