...
public class PulseHandlerCC extends PulseHandler {
	...
	/**
	 * Constant that indicates which strategy found the final path (MT = 1 , MC =
	 * 2,MD = 3)
	 */
	private int best = 1;
	...
	public void runPulse(int beg, int last, double timeC, double distC, String endNode, int se) {
		 ...
		 * System.out.println(arc.getTail().getId()+" - "+arc.getHead().getId()+" -
		 * "+arc.getCost()+" - "+arc.getCost()+" - "+arc.getWalk_time());
		 * }
		 * }
		 * }
		 */

		// Create an auxiliary node (The node alpha) an his arcs. In addition it sets
		// the minimum time and the minimum cost
		...
		// Reset pending pulses. This is relevant because of the strategy we have..where
		// we don't create the network from scratch
		...
		if (node.getMinTime()[1] > timeC || node.getMinDist()[2] > distC) { // Check if is infeasible by any of the
																			// resources
	...
	public ArrayList<String> recoverFrontMT(int beg) {
		...
		while (termine == false) {
			...
			for (Iterator<PulseArc> iter = actNode.getIncomingArcs().iterator(); iter.hasNext() && alt == false;) {
				...
				if (arc.getWalk_time() + finalDist + arc.getTail().getMinTime()[2] - distStar < Math.pow(10,
						-globalParameters.GlobalParameters.PRECISION)) {
					if (arc.getTime() + finalTime + arc.getTail().getMinTime()[1] - timeStar < Math.pow(10,
							-globalParameters.GlobalParameters.PRECISION)) {
						if (arc.getCost() + finalCost + arc.getTail().getMinTime()[0] - primalBound < Math.pow(10,
								-globalParameters.GlobalParameters.PRECISION)) {
	...
	public ArrayList<String> recoverFrontMD(int beg) {
		...
		while (termine == false) {
			...
			for (Iterator<PulseArc> iter = actNode.getIncomingArcs().iterator(); iter.hasNext() && alt == false;) {
				PulseArc arc = iter.next();
				if (arc.getWalk_time() + finalDist + arc.getTail().getMinDist()[2] - distStar < Math.pow(10,
						-globalParameters.GlobalParameters.PRECISION)) {
					if (arc.getTime() + finalTime + arc.getTail().getMinDist()[1] - timeStar < Math.pow(10,
							-globalParameters.GlobalParameters.PRECISION)) {
						if (arc.getCost() + finalCost + arc.getTail().getMinDist()[0] - primalBound < Math.pow(10,
								-globalParameters.GlobalParameters.PRECISION)) {
							idAct = arc.getTail().getId();
							finalTime += arc.getTime();
							finalCost += arc.getCost();
	...
	public ArrayList<String> recoverFrontMC(int beg) {
		...
		while (termine == false) {
			...
			for (Iterator<PulseArc> iter = actNode.getIncomingArcs().iterator(); iter.hasNext() && alt == false;) {
				PulseArc arc = iter.next();
				if (arc.getWalk_time() + finalDist + arc.getTail().getMinCost()[2] - distStar < Math.pow(10,
						-globalParameters.GlobalParameters.PRECISION)) {
					if (arc.getTime() + finalTime + arc.getTail().getMinCost()[1] - timeStar < Math.pow(10,
							-globalParameters.GlobalParameters.PRECISION)) {
						if (arc.getCost() + finalCost + arc.getTail().getMinCost()[0] - primalBound < Math.pow(10,
								-globalParameters.GlobalParameters.PRECISION)) {
							idAct = arc.getTail().getId();
							finalTime += arc.getTime();
							finalCost += arc.getCost();
	...
	public ArrayList<String> recoverBackPart(int beg) {
		...
		while (termine == false) {
			...
			for (int i = 0; i < pendingPulses.size() && !cambie; i++) {
				...
				if (Math.abs(p.getCost() + costoAcumulado - primalBound) < Math.pow(10,
						-globalParameters.GlobalParameters.PRECISION)) {
					if (Math.abs(p.getTime() + tiempoAcumulado - timeStar) < Math.pow(10,
							-globalParameters.GlobalParameters.PRECISION)) {
						if (Math.abs(p.getDist() + distAcumulado - distStar) < Math.pow(10,
								-globalParameters.GlobalParameters.PRECISION)) {
							...
							for (Iterator<PulseArc> iter = actNode.getOutgoingArcs().iterator(); iter.hasNext()
								...
								if (arc.getHead().getId().equals(nodoActual)) {
									...
									for (int ii = 0; ii < pendingPulsesAux.size(); ii++) {
										...
										if (Math.abs(pp.getCost() + costoAcumulado + arc.getCost() - primalBound) < Math
												.pow(10, -globalParameters.GlobalParameters.PRECISION)) {
											if (Math.abs(
													pp.getTime() + tiempoAcumulado + arc.getTime() - timeStar) < Math
															.pow(10, -globalParameters.GlobalParameters.PRECISION)) {
												if (Math.abs(pp.getDist() + distAcumulado + arc.getWalk_time()
														- distStar) < Math.pow(10,
																-globalParameters.GlobalParameters.PRECISION)) {
		...
		if (Math.abs(costoAcumulado - primalBound) > Math.pow(10, -globalParameters.GlobalParameters.PRECISION)
				|| Math.abs(tiempoAcumulado - timeStar) > Math.pow(10, -globalParameters.GlobalParameters.PRECISION)
				|| Math.abs(distAcumulado - distStar) > Math.pow(10, -globalParameters.GlobalParameters.PRECISION)) {
			termine = true;
			modifyValues();
		}
		return path;
