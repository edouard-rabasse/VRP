...
def process_image_pairs(
    ...
    original_files = [
        f for f in os.listdir(original_dir) if f.lower().endswith(img_extensions)
    ]
...
def get_mask_pixelised(original_image, modified_image, pixel_size=10):
    ...
    macro_mask = np.zeros(
        (mask.shape[0] // pixel_size, mask.shape[1] // pixel_size), dtype=np.uint8
    )
    for i in range(macro_mask.shape[0]):
        for j in range(macro_mask.shape[1]):
            macro_mask[i, j] = np.max(
                mask[
...
def get_removed_lines(original_image, modified_image, colored=True):
    ...
    lower_color = np.array(
        [0, 30, 30]
    )  # increase lower saturation and value to avoid near-white
